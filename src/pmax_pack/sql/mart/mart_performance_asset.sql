BEGIN TRANSACTION;
DELETE FROM `{{ project }}.{{ marts_dataset }}.mart_performance_asset` WHERE date BETWEEN DATE_SUB(@as_of, INTERVAL {{ window_days }} DAY) AND @as_of;
INSERT INTO `{{ project }}.{{ marts_dataset }}.mart_performance_asset`
