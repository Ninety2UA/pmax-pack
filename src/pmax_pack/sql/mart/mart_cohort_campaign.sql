BEGIN TRANSACTION;
DELETE FROM `{{ project }}.{{ marts_dataset }}.mart_cohort_campaign`
WHERE click_date BETWEEN DATE_SUB(@as_of, INTERVAL {{ window_days }} DAY) AND @as_of;
INSERT INTO `{{ project }}.{{ marts_dataset }}.mart_cohort_campaign`
WITH costs AS (
  SELECT
    date AS click_date,
    account_id,
...
SELECT
  ...
  c.source_run_id,
  @run_id AS run_id
FROM `{{ project }}.{{ marts_dataset }}.int_lag_prefix_campaign` AS c
